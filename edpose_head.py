...
class EDPoseOutHead(BaseModule):
    ...
    def dn_post_process2(self, outputs_class, outputs_coord, outputs_keypoints_list, mask_dict):
        ...
        Args:
            outputs_class (List[Tensor]): Classification outputs for each decoder layer.
            outputs_coord (List[Tensor]): Coordinate outputs for each decoder layer.
            outputs_keypoints_list (List[Tensor]): Keypoint outputs for each decoder layer.
            mask_dict (Dict): Dictionary containing mask information for denoising.
        Returns:
            Tuple[List[Tensor]]: Processed outputs after denoising.
                - outputs_class: Processed classification outputs.
                - outputs_coord: Processed coordinate outputs.
                - outputs_keypoint: Processed keypoint outputs.
        """
    ...
    def loss(self,
        ...
        for data_sample in batch_data_samples:
            ...
            if isinstance(gt_instances.bboxes, np.ndarray):
                ...
                bboxes_tensor = gt_instances.bboxes
            x1, y1, x2, y2 = bboxes_tensor.unbind(-1)
            ...
            # 首先填充所有x和y坐标，然后填充所有可见性值
            for i in range(num_keypoints):
                # 归一化坐标到[0,1]范围
                keypoints_flat[:, i*2] = keypoints[:, i, 0] / img_w      # 归一化x坐标
                keypoints_flat[:, i*2+1] = keypoints[:, i, 1] / img_h     # 归一化y坐标
