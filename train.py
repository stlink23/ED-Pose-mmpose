...
def parse_args():
    ...
    parser.add_argument('--config', default='mmpose\\configs\\body_2d_keypoint\\edpose\\coco\\my_edpose_res50_8xb2-50e_coco-800x1333.py',help='train config file path'  )
...
def main():
    ...
    # 添加对 bbox_file 的检查
    if 'val_dataloader' in cfg and cfg.val_dataloader is not None:
        dataset_cfg = cfg.val_dataloader.dataset
        if hasattr(dataset_cfg, 'bbox_file') and dataset_cfg.bbox_file:
            try:
                # 尝试加载 JSON 文件进行验证
                with open(dataset_cfg.bbox_file, 'r') as f:
                    json.load(f)
            except json.JSONDecodeError as e:
                print(f"错误: bbox_file '{dataset_cfg.bbox_file}' 包含无效的 JSON 格式")
                print(f"JSON 错误: {str(e)}")
                print("请检查 JSON 文件格式，确保其符合标准 JSON 格式")
                print("您可以使用在线 JSON 验证工具来检查和修复文件")
                return
